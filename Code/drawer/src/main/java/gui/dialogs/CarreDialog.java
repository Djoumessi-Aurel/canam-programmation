...
public class CarreDialog extends JDialog implements ActionListener{
	...
	public CarreDialog(JFrame parent, int xCentre, int yCentre) {
		super(parent);
		setModal(true);
		setTitle("Dessiner un carré");
	...
	public void actionPerformed(ActionEvent arg0) {
        ...
        Carre carre = new Carre(cote, xCentre, yCentre, rot, color);

        if(shapeIndex == -1){
            Appli.createShape(carre);
        }
        else{
            Appli.modifyShape(shapeIndex, carre);
        }

        dispose();
	}
	
	public void gererModification(int shapeIndex, Carre carre)//Modifications à faire lorsque la fenêtre va servir à MODIFIER les informations
	{
        this.shapeIndex = shapeIndex;

		this.setTitle("Modifier le carré");
        spinnerCote.setValue(carre.getCote());
        spinnerRot.setValue(new Double(carre.getAngleRotation()).intValue());
        spinnerX.setValue(carre.getXCentre());
        spinnerY.setValue(carre.getYCentre());
		labelCouleur.setBackground(carre.getColor());
	}
