...
public class EllipseDialog extends JDialog implements ActionListener{
	...
	public EllipseDialog(JFrame parent, int xCentre, int yCentre) {
		super(parent);
		setModal(true);
		setTitle("Dessiner une ellipse");
	...
	public void actionPerformed(ActionEvent arg0) {
        ...
        Ellipse ellipse = new Ellipse(largeur, hauteur, xCentre, yCentre, rot, color);

        if(shapeIndex == -1){
            Appli.createShape(ellipse);
        }
        else{
            Appli.modifyShape(shapeIndex, ellipse);
        }

        dispose();
	}
	
	public void gererModification(int shapeIndex, Ellipse ellipse)//Modifications à faire lorsque la fenêtre va servir à MODIFIER les informations
	{
        this.shapeIndex = shapeIndex;

		this.setTitle("Modifier l'ellipse");
        spinnerLargeur.setValue(ellipse.getLargeur());
        spinnerHauteur.setValue(ellipse.getHauteur());
        spinnerRot.setValue(new Double(ellipse.getAngleRotation()).intValue());
        spinnerX.setValue(ellipse.getXCentre());
        spinnerY.setValue(ellipse.getYCentre());
		labelCouleur.setBackground(ellipse.getColor());
	}
