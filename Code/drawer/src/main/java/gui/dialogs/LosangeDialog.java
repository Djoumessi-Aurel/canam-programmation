...
public class LosangeDialog extends JDialog implements ActionListener{

	private static final long serialVersionUID = 1L;
	private final JPanel contentPanel = new JPanel();

    private JSpinner spinnerDiagX, spinnerDiagY, spinnerX, spinnerY, spinnerRot;
	 ...
	 * Create the dialog.
	 */
	public LosangeDialog(JFrame parent, int xCentre, int yCentre) {
		super(parent);
		setModal(true);
		setTitle("Dessiner un losange");
        ...
        spinnerDiagX = new JSpinner(new SpinnerNumberModel(80, 1, 1000, 1));
        spinnerDiagY = new JSpinner(new SpinnerNumberModel(40, 1, 1000, 1));
...

		{
			...
			contentPanel.add(panel1);
			{
				JLabel labelNom = new JLabel("Diagonale horizontale: ");
				panel1.add(labelNom);
                panel1.add(spinnerDiagX);
                labelNom.addMouseListener(new FocusComponentListener(spinnerDiagX));
			}
		}
        {
			JPanel panel1 = new JPanel();
			...
			contentPanel.add(panel1);
			{
				JLabel labelNom = new JLabel("Diagonale verticale: ");
				panel1.add(labelNom);
                panel1.add(spinnerDiagY);
                labelNom.addMouseListener(new FocusComponentListener(spinnerDiagY));
			}
		}
		{
	...
	public void actionPerformed(ActionEvent arg0) {
        ...
        int diagX = (int)spinnerDiagX.getValue();
        int diagY = (int)spinnerDiagY.getValue();
        ...
        Losange losange = new Losange(diagX, diagY, xCentre, yCentre, rot, color);

        if(shapeIndex == -1){
            Appli.createShape(losange);
        }
        else{
            Appli.modifyShape(shapeIndex, losange);
        }

        dispose();
	}
	
	public void gererModification(int shapeIndex, Losange losange)//Modifications à faire lorsque la fenêtre va servir à MODIFIER les informations
	{
        this.shapeIndex = shapeIndex;

		this.setTitle("Modifier le losange");
        spinnerDiagX.setValue(losange.getDiagonaleX());
        spinnerDiagY.setValue(losange.getDiagonaleY());
        spinnerRot.setValue(new Double(losange.getAngleRotation()).intValue());
        spinnerX.setValue(losange.getXCentre());
        spinnerY.setValue(losange.getYCentre());
		labelCouleur.setBackground(losange.getColor());
	}
